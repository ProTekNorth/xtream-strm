...
function renderStatus(s){const p=s.progress||{},total=Number(p.total||0),current=Number(p.current||0),percent=total?Math.min(100,current/total*100):0;$('#stage').textContent=p.stage||'Ready';$('#detail').textContent=p.detail||(s.running?'Sync is running':'No sync is running');$('#progressBar').style.width=percent+'%';$('#progressText').textContent=total?`${current.toLocaleString()} of ${total.toLocaleString()} — ${percent.toFixed(1)}%`:(s.running?'Working…':p.state==='complete'?'Complete':'Ready');$('#statePill').textContent=s.running?'Syncing':p.state==='failed'?'Needs attention':'Ready';$('#statePill').classList.toggle('running',s.running);$('#stopSync').disabled=!s.running;$('#timerToggle').checked=s.timer_enabled;$('#timerMetric').textContent=s.timer_enabled?'Enabled':'Disabled';$('#miniLogs').textContent=s.logs||'No activity yet.';$('#fullLogs').textContent=s.logs||'No activity yet.';$('#unitName').textContent=s.unit||'';for(const e of [$('#miniLogs'),$('#fullLogs')])e.scrollTop=e.scrollHeight}
